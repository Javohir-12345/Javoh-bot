...
async def process_song_genre(message: Message, state: FSMContext):
    ...
    if use_tariff:
        # Qayta tekshirib olamiz (holat o'zgargan bo'lishi mumkin)
        status = get_tariff_status(message.from_user.id)
        if use_bonus:
            ...
            db_use_bonus_song(message.from_user.id, today_str)
        else:
            ...
            db_decrement_tariff_song(message.from_user.id)
    ...
    try:
        ...
        # Foydalanuvchiga qancha qoldi degan ma'lumot
        extra_info = ""
        if use_tariff and not use_bonus:
            new_status = get_tariff_status(message.from_user.id)
            extra_info = f"\n\n🎼 Tarif bo'yicha qolgan qo'shiqlar: {new_status['songs_left']} ta"
        elif not use_tariff:
            remaining_balance = db_get_user(message.from_user.id)[0]
